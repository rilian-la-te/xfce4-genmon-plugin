...
/* Execute the onClick Command */
{
  ...
  bool fail = vala_panel_launch_command_on_screen(poMonitor->onClickCmd,
                                                  poPlugin->plugin);
  if (fail) {
    g_autofree char *first =
        g_strdup_printf(_("Could not run \"%s\""), poMonitor->onClickCmd);
    g_autoptr(GtkDialog) dlg = GTK_DIALOG(gtk_message_dialog_new(
        NULL, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR,
        GTK_BUTTONS_CLOSE, "%s", first));
    gtk_dialog_run(dlg);
  }
}
...
/* Execute the onClick Command */
{
  ...
  bool fail = vala_panel_launch_command_on_screen(poMonitor->onValClickCmd,
                                                  poPlugin->plugin);
  if (fail) {
    g_autofree char *first =
...
/* Create the plugin */
{
  ...
  GtkOrientation orientation =
      gtk_orientable_get_orientation(GTK_ORIENTABLE(plugin));
  ...
  gtk_container_add(GTK_CONTAINER(poMonitor->wValButton),
                    poMonitor->wValButtonLabel);
  ...
  gtk_box_pack_start(GTK_BOX(poMonitor->wBox), GTK_WIDGET(poMonitor->wBar),
                     FALSE, FALSE, 0);
   ...
   previously stored in xml file */
{
  ...
  poConf->fTitleDisplayed =
      xfce_rc_read_int_entry(rc, (GENMON_PROP_USE_LABEL), 1);
  if (poConf->fTitleDisplayed)
  ...
  poConf->iPeriod_ms =
      xfce_rc_read_int_entry(rc, (GENMON_PROP_UPDATE_PERIOD), 30 * 1000);
...
/* Create/pop up the configuration/options GUI */
{
  ...
  g_signal_connect(GTK_WIDGET(poGUI->wTF_Cmd), "activate", G_CALLBACK(SetCmd),
                   poPlugin);
...
static void genmon_set_orientation(GenMonWidget *plugin,
...
/**************************************************************/
static gboolean genmon_set_size(GenMonWidget *plugin, int size,
                                genmon_t *poPlugin)
/* Plugin API */
/* Set the size of the panel-docked monitor */
{
  ...
  if (gtk_orientable_get_orientation(GTK_ORIENTABLE(plugin)) ==
      GTK_ORIENTATION_HORIZONTAL) {
    if (size > BORDER)
      gtk_widget_set_size_request(GTK_WIDGET(poMonitor->wBar), 8,
                                  size - BORDER * 2);
  } else {
    if (size > BORDER)
...
static gboolean genmon_remote_event(GenMonWidget *plugin, const gchar *name,
                                    const GValue *value, genmon_t *genmon) {
...
static void genmon_construct(GenMonWidget *plugin) {
  ...
  g_signal_connect(plugin, "remote-event", G_CALLBACK(genmon_remote_event),
                   genmon);
