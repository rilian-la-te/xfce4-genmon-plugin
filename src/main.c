   ...
   text field */
{
	...
	if (begin && end && begin < end)
	{
		...
		if (value > 100)
			value = 100;
		gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(poMonitor->wBar),
		                              (float)value / 100.0);
...
static genmon_t *genmon_create_control(GenMonWidget *plugin)
/* Plugin API */
/* Create the plugin */
{
	...
	GtkOrientation orientation = gtk_orientable_get_orientation(GTK_ORIENTABLE(plugin));
	...
	/* Add Value Button */
	poMonitor->wValButton = gtk_button_new();
	...
	gtk_box_pack_start(GTK_BOX(poMonitor->wImgBox),
	                   GTK_WIDGET(poMonitor->wValButton),
	                   TRUE,
	                   FALSE,
	                   0);
	...
	gtk_box_pack_start(GTK_BOX(poMonitor->wBox), GTK_WIDGET(poMonitor->wBar), FALSE, FALSE, 0);
	if (orientation == GTK_ORIENTATION_HORIZONTAL)
	{
		gtk_orientable_set_orientation(GTK_ORIENTABLE(poMonitor->wBar),
...
static void genmon_free(GenMonWidget *plugin, genmon_t *poPlugin)
/* Plugin API */
{
...
static int SetMonitorFont(void *p_pvPlugin)
{
	...
	gtk_style_context_add_provider(GTK_STYLE_CONTEXT(gtk_widget_get_style_context(
	                               ...
	                               GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
	gtk_style_context_add_provider(GTK_STYLE_CONTEXT(gtk_widget_get_style_context(
	                                   GTK_WIDGET(poMonitor->wValButtonLabel))),
	                               GTK_STYLE_PROVIDER(css_provider),
	                               GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
	g_free(css);
...
static void genmon_read_config(GenMonWidget *plugin, genmon_t *poPlugin)
/* Plugin API */
   ...
   previously stored in xml file */
{
	...
	if ((pc = xfce_rc_read_entry(rc, (GENMON_PROP_CMD), NULL)))
	{
		g_free(poConf->acCmd);
		poConf->acCmd = g_strdup(pc);
	}

	poConf->fTitleDisplayed = xfce_rc_read_int_entry(rc, (GENMON_PROP_USE_LABEL), 1);
	if (poConf->fTitleDisplayed)
		gtk_widget_show(GTK_WIDGET(poMonitor->wTitle));
	else
		gtk_widget_hide(GTK_WIDGET(poMonitor->wTitle));

	if ((pc = xfce_rc_read_entry(rc, (GENMON_PROP_LABEL_TEXT), NULL)))
	{
	...
	poConf->iPeriod_ms = xfce_rc_read_int_entry(rc, (GENMON_PROP_UPDATE_PERIOD), 30 * 1000);

	if ((pc = xfce_rc_read_entry(rc, (GENMON_PROP_FONT), NULL)))
	{
...
static void genmon_write_config(GenMonWidget *plugin, genmon_t *poPlugin)
/* Plugin API */
/* Write plugin configuration into xml file */
{
	...
	xfce_rc_write_entry(rc, GENMON_PROP_CMD, poConf->acCmd);

	xfce_rc_write_int_entry(rc, GENMON_PROP_USE_LABEL, poConf->fTitleDisplayed);

	xfce_rc_write_entry(rc, GENMON_PROP_LABEL_TEXT, poConf->acTitle);

	xfce_rc_write_int_entry(rc, GENMON_PROP_UPDATE_PERIOD, poConf->iPeriod_ms);

	xfce_rc_write_entry(rc, GENMON_PROP_FONT, poConf->acFont);
...
/* GUI callback turning on/off the monitor bar legend */
{
	...
	gtk_widget_set_sensitive(GTK_WIDGET(poGUI->wTF_Title), poConf->fTitleDisplayed);
	if (poConf->fTitleDisplayed)
		gtk_widget_show(GTK_WIDGET(poMonitor->wTitle));
	else
		gtk_widget_hide(GTK_WIDGET(poMonitor->wTitle));
...
static void About(GenMonWidget *plugin)
{
	//	GdkPixbuf *icon;
	...
	//	icon = xfce_panel_pixbuf_from_source("utilities-system-monitor", NULL,
	// 32);
	...
	// displays "
	//	      "the resulting string in the panel"),
	//	    "website",
...
static void ChooseFont(GtkWidget *p_wPB, void *p_pvPlugin)
{
	...
	gtk_window_set_transient_for(GTK_WINDOW(wDialog), GTK_WINDOW(poPlugin->oConf.wTopLevel));
...
static void genmon_dialog_response(GtkWidget *dlg, int response, genmon_t *genmon)
{
	UpdateConf(genmon);
...
static void genmon_create_options(GenMonWidget *plugin, genmon_t *poPlugin)
/* Plugin API */
/* Create/pop up the configuration/options GUI */
{
	...
	gtk_widget_set_sensitive(GTK_WIDGET(poGUI->wTF_Title), poConf->fTitleDisplayed);
	g_signal_connect(GTK_WIDGET(poGUI->wTB_Title),
	                 "toggled",
	                 G_CALLBACK(ToggleTitle),
	                 poPlugin);

	gtk_entry_set_text(GTK_ENTRY(poGUI->wTF_Cmd), poConf->acCmd);
	g_signal_connect(GTK_WIDGET(poGUI->wTF_Cmd), "activate", G_CALLBACK(SetCmd), poPlugin);
	...
	g_signal_connect(GTK_WIDGET(poGUI->wSc_Period),
	                 "value_changed",
	                 G_CALLBACK(SetPeriod),
	                 poPlugin);

	if (strcmp(poConf->acFont, "(default)")) /* Default font */
		gtk_button_set_label(GTK_BUTTON(poGUI->wPB_Font), poConf->acFont);
	g_signal_connect(G_OBJECT(poGUI->wPB_Font), "clicked", G_CALLBACK(ChooseFont), poPlugin);
...
static void genmon_set_orientation(GenMonWidget *plugin, GtkOrientation p_iOrientation,
                                   genmon_t *poPlugin)
/* Plugin API */
...
/**************************************************************/
static gboolean genmon_set_size(GenMonWidget *plugin, int size, genmon_t *poPlugin)
/* Plugin API */
...
static gboolean genmon_remote_event(GenMonWidget *plugin, const gchar *name, const GValue *value,
                                    genmon_t *genmon)
{
...
static void genmon_construct(GenMonWidget *plugin)
{
	genmon_t *genmon;
	...
	g_signal_connect(plugin, "orientation-changed", G_CALLBACK(genmon_set_orientation), genmon);
	...
	g_signal_connect(G_OBJECT(genmon->oMonitor.wButton),
	                 "clicked",
	                 G_CALLBACK(ExecOnClickCmd),
	                 genmon);

	g_signal_connect(G_OBJECT(genmon->oMonitor.wValButton),
	                 "clicked",
